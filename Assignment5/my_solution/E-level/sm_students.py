...
class StateMachine(object):
    def __init__(self):
        ...
        self.cube_pose_param = rospy.get_param(rospy.get_name() + '/cube_pose')
        self.cube_pose_param = [float(x) for x in self.cube_pose_param.split(',')]
        ...
        while not rospy.is_shutdown() and self.state != -1:
            ...
            if self.state == 1:
                try:
                    ...
                    # get the pose message of the cube
                    cube_pose_msg = PoseStamped()
                    cube_pose_msg.header.stamp = rospy.Time.now()
                    cube_pose_msg.header.frame_id = self.robot_base_frame
                    cube_pose_msg.pose.position.x = self.cube_pose_param[0]
                    cube_pose_msg.pose.position.y = self.cube_pose_param[1]
                    cube_pose_msg.pose.position.z = self.cube_pose_param[2]
                    cube_pose_msg.pose.orientation.x = self.cube_pose_param[3]
                    cube_pose_msg.pose.orientation.y = self.cube_pose_param[4]
                    cube_pose_msg.pose.orientation.z = self.cube_pose_param[5]
                    cube_pose_msg.pose.orientation.w = self.cube_pose_param[6]
                    print("Got the cube pose")
                    self.cube_pose_pub.publish(cube_pose_msg)
                    print("Published the cube pose")
                    pick_srv = rospy.ServiceProxy(self.pick_srv_nm, SetBool)
                    print("Waiting for pick service response...")
                    pick_req = pick_srv()
            ...
            # State 2: Move the robot to second table
            if self.state == 2:
                rospy.loginfo("%s: Moving towards second table", self.node_name)
            ...
            if self.state == 3:
                try:
                    ...
                    place_srv_ = rospy.ServiceProxy(self.place_srv_nm, SetBool)
