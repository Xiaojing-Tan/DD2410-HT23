    ...
    def update_map(self, grid_map, pose, scan):
        ...
        for i in range(len(scan.ranges)):
            if scan.range_min < scan.ranges[i] < scan.range_max:
                ...
                x_map = scan.ranges[i] * cos(angle + robot_yaw) + pose.pose.position.x - origin.position.x
                y_map = scan.ranges[i] * sin(angle + robot_yaw) + pose.pose.position.y - origin.position.y
                x_index = int(x_map / resolution)
                y_index = int(y_map / resolution)
                obstacles.append((x_index, y_index))
                # C part: fill in the free cells
                start = (int((pose.pose.position.x - origin.position.x) / resolution),
                         int((pose.pose.position.y - origin.position.y) / resolution))
                end = (x_index, y_index)
        ...
        update.x = min([obstacle[0] for obstacle in obstacles])
        # The minimum y index in 'grid_map' that has been updated
        update.y = min([obstacle[1] for obstacle in obstacles])
        # Maximum x index - minimum x index + 1
        update.width = max([obstacle[0] for obstacle in obstacles]) - min([obstacle[0] for obstacle in obstacles]) + 1
        # Maximum y index - minimum y index + 1
        update.height = max([obstacle[1] for obstacle in obstacles]) - min([obstacle[1] for obstacle in obstacles]) + 1
                ...
                if grid_map[x0, y0] == self.occupied_space:
                    for x in range(x0 - self.radius, x0 + self.radius + 1):
                        for y in range(y0 - self.radius, y0 + self.radius + 1):
                            if self.is_in_bounds(grid_map, x, y) and grid_map[x, y] != self.occupied_space and sqrt((x-x0)**2+(y-y0)**2) <= self.radius:
